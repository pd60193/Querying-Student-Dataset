...
class Evaluator:
    ...
    def query_data_source(self, query: str) -> Optional[pd.DataFrame]:
        try:
            # Basic sanitization
...
def load_metrics(filepath: str) -> List[ModelResponse]:
    try:
        with open(filepath, 'r', encoding='utf-8') as file:
            ...
            return [ModelResponse(**item) for item in data]
...
def main():
    ...
    # Note: grammar.js provided in source contains JS code `export const...`. 
    # Python Lark reads raw string. You might need a separate .lark file or strip JS syntax in Python.
    
    # Assuming we point to a raw .lark file for Python:
    args = parser.parse_args()

    # Determine absolute paths
    evaluator = Evaluator(grammar_path)

    metrics = load_json_metrics(args.metrics)
...
print(f"Accuracy (Fraction of all generations that are factually right): {acc:.2%} of total generations\n")
...
print(f"Hallucinations (Fraction of completed generations that are invalid sql grammar i.e. nonsensical): {hallucination_rate:.2%}")
...
print(f"ools Usage (Fraction of attempted calls in which the model called a custom tool): {tool_usage_rate:.2%}")
